...
public interface ItemRepository extends JpaRepository<Item, Integer> {
	...
	Page<Item> findByDeleteFlagOrderByInsertDateDescPage(@Param(value = "deleteFlag") int deleteFlag,
			Pageable pageable);
	...
	Page<Item> findByCategoryOrderByIdDesc(@Param(value = "deleteFlag") int deleteFlag,
...
//	List<Item> findAllByQuery(@Param(value = "deleteFlag") int deleteFlag);
	 ...
	 */
	@Query("SELECT i FROM Item i LEFT JOIN OrderItem o ON o.item.id=i.id WHERE i.deleteFlag = :deleteFlag AND i.category.id=:categoryId GROUP BY i ORDER BY COALESCE(SUM(o.quantity),0) DESC, i.id ASC")
	Page<Item> findCategoryByQuery(@Param(value = "deleteFlag") int deleteFlag,
			@Param(value = "categoryId") Integer categoryId, Pageable pageable);
